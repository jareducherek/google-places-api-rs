...
mod nearby_search {
    use super::*;
    pub async fn build_nearby_search(
        api_key: &str,
        location: &(f64, f64),
        radius: Option<&u32>,
        keyword: Option<&str>,
    ...
    pub async fn process_nearby_search(body: &String) -> Result<NearbySearchResult, GooglePlacesError>{
...
mod text_search {
    use super::*;
    pub async fn process_text_search(body: &String) -> Result<TextSearchResult, GooglePlacesError> {
    ...
    pub async fn build_text_search(
        api_key: &str,
        query: &str,
        radius: &u32,
        language: Option<&Language>,
...
mod find_place {
    use super::*;
    pub async fn process_find_place(body: &String) -> Result<FindPlaceSearchResult, GooglePlacesError>{
...
impl PlaceSearchService {
    ...
    async fn full_nearby_search(
        ...
        let url = nearby_search::build_nearby_search(self.client.get_api_key(), location, radius, keyword, language, max_price, min_price, open_now, page_token, rank_by, place_types).await?;
        ...
        Ok(nearby_search::process_nearby_search(&body).await?)
    }
    ...
    pub async fn find_place(
        ...
        let url = find_place::build_find_place(self.client.get_api_key(), input, input_type, fields, language, location_bias).await?;
        let response: reqwest::Response = match self.client.get_req_client().get(url).send().await{
            Ok(response) => response,
            Err(e) => return Err(GooglePlacesError::HttpError(e)),
        };
        ...
        Ok(find_place::process_find_place(&body).await?)
    }
    ...
    pub async fn text_search(
        ...
        let url = text_search::build_text_search(self.client.get_api_key(), query, radius, language, location, max_price, min_price, open_now, page_token, region, place_types).await?;
        let response: reqwest::Response = match self.client.get_req_client().get(url).send().await{
            Ok(response) => response,
            Err(e) => return Err(GooglePlacesError::HttpError(e)),
        };
        ...
        Ok(text_search::process_text_search(&body).await?)
    }
}
...
mod test{
    use super::*;
    #[tokio::test]
    async fn test_build_nearby_search() {
        // Define test inputs
        let api_key = "12345";
        ...
        //let page_token = Some("next-page-token");
        ...
        let url = nearby_search::build_nearby_search(api_key, location, radius, keyword, language, max_price, min_price, open_now, None, rank_by, place_types).await.unwrap();
        let actual_url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=0%2C0&key=12345&radius=1000&keyword=restaurant&language=en&maxprice=2&minprice=1&opennow=true&rankby=distance&type=restaurant,establishment,food".to_string();
        assert_eq!(url, actual_url);
    }

    #[tokio::test]
    async fn test_build_text_search() {
        // Define test inputs
        let api_key = "12345";
        let query = "query";
        let radius = Some(&1000);
        ...
        let place_types = Some(&place_set);
        let url = text_search::build_text_search(api_key, &query, radius, keyword, language, max_price, min_price, open_now, None, rank_by, place_types).await.unwrap();
        let actual_url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=0%2C0&key=12345&radius=1000&keyword=restaurant&language=en&maxprice=2&minprice=1&opennow=true&rankby=distance&type=restaurant,establishment,food".to_string();
        assert_eq!(url, actual_url);
    }
}